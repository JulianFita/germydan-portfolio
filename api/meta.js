import fetch from 'node-fetch'

const TMDB_API_KEY = process.env.TMDB_API_KEY || ''
const OMDB_API_KEY = process.env.OMDB_API_KEY || ''
const LIBRE_API_URL = process.env.LIBRE_API_URL || 'https://libretranslate.com'
const LIBRE_API_KEY = process.env.LIBRE_API_KEY || ''

async function translateToSpanish(text) {
  if (!text) return text
  try {
    const body = { q: text, source: 'auto', target: 'es', format: 'text' }
    if (LIBRE_API_KEY) body.api_key = LIBRE_API_KEY
    const r = await fetch(`${LIBRE_API_URL}/translate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body)
    })
    if (!r.ok) throw new Error('Error en traducción')
    const data = await r.json()
    return data.translatedText || text
  } catch (e) {
    return text
  }
}

async function fetchFromTMDBByImdb(imdbId) {
  if (!TMDB_API_KEY) throw new Error('TMDB_API_KEY no configurada')
  const r = await fetch(`https://api.themoviedb.org/3/find/${imdbId}?api_key=${TMDB_API_KEY}&language=en-US&external_source=imdb_id`)
  if (!r.ok) throw new Error('Error al buscar en TMDB')
  const j = await r.json()
  return j.movie_results?.[0] || j.tv_results?.[0] || {}
}

async function fetchFromOmdb(imdbId) {
  if (!OMDB_API_KEY) throw new Error('OMDB_API_KEY no configurada')
  const url = `http://www.omdbapi.com/?i=${imdbId}&plot=full&apikey=${OMDB_API_KEY}`
  const r = await fetch(url)
  if (!r.ok) throw new Error('OMDB request failed')
  const j = await r.json()
  if (j.Response === 'False') throw new Error(j.Error || 'No encontrado')
  return j
}

export default async function handler(req, res) {
  const { id } = req.query
  if (!id) return res.status(400).json({ error: 'Falta parámetro id' })

  let originalOverview = 'Descripción no disponible'
  let title = id
  let year = ''
  let type = 'movie'

  try {
    if (OMDB_API_KEY && id.startsWith('tt')) {
      const omdbData = await fetchFromOmdb(id)
      originalOverview = omdbData.Plot || originalOverview
      title = omdbData.Title || title
      year = omdbData.Year || ''
      type = omdbData.Type === 'series' ? 'series' : 'movie'
    } else if (TMDB_API_KEY && id.startsWith('tt')) {
      const tmdbData = await fetchFromTMDBByImdb(id)
      originalOverview = tmdbData.overview || originalOverview
      title = tmdbData.title || tmdbData.name || title
      year = (tmdbData.release_date || tmdbData.first_air_date || '').split('-')[0] || ''
      type = tmdbData.name ? 'series' : 'movie'
    }

    const translated = await translateToSpanish(originalOverview)

    return res.status(200).json({
      id,
      name: title,
      type,
      description: translated,
      descriptionFull: translated,
      year
    })
  } catch (err) {
    return res.status(500).json({ error: err.message })
  }
}
